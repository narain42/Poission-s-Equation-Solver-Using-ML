...
##    npr.shuffle(points)
    ...
    for iters in range(num_epochs):   
        ...
        ##      W = pickle.load(open('W_min_sim_adam_bfgs', 'rb'))
        ...
        W = adam( objective_grad, W, step_size=step_size,
               num_iters= num_batches, callback=printPerf)
        ...
        optim_W = minimize(error_part, x0=flattened_W, jac=error_part_grad, method='L-BFGS-B', options={'gtol': 1e-15, 'maxiter':5})
    ...
    W = unflat_func(o.x)
    pickle.dump(W, open('W_min_sim_adam_bfgs', 'wb'), protocol=4)
